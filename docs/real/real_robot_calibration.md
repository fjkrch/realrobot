...
## Jetson To Isaac Lab Mirror First

Use this before commanding the real robot. Commands typed on the Jetson are sent
to the laptop and mirrored in Isaac Lab / Isaac Sim only.

Instruction file:

```text
/home/chyanin/Desktop/realrobot/docs/sim/REAL_JETSON_TO_ISAAC_SIMULATION.md
```

Best laptop server for visualization, Isaac stays open:
...
This clean default-pose server is separate from the Isaac task project. It opens
only OpenArm, ground, and light. It has no cube, no pick state machine, and no
episode reset.

Older task-based laptop server for visualization:
...
This live server holds the simulated arm in a stable ready/default pose and
disables the Isaac Lab timeout/drop resets by default. It should stay open and
not reset every few seconds while waiting for Jetson commands. Use
`ALLOW_AUTO_RESETS=1` only if you intentionally want the original training
resets back for debugging.

Older laptop server, launches Isaac per command:
...
Do not run both laptop servers on port `8765`. For watching the robot in Isaac,
use `run_openarm_live_command_server.txt`.

### Real Gripper Command Also Mirrors

The Jetson script below now sends a best-effort numeric Isaac mirror command
before sending the real gripper action:
...
`--target-deg 0` mirrors as:

```text
gripper target 0.000 deg
```

Negative targets like `--target-deg -10` mirror as:

```text
gripper target -10.000 deg
```

Isaac maps `0..-65 deg` to `0.000..0.044 m/finger`, so `-10 deg` is about
`0.0068 m/finger` in simulation.
