...
| dataset v1 export | ok | 4.7 |
| train v1 prepare | ok | 7.3 |
| eval v1 manifest | ok | 0.0 |
| dataset v2 export | ok | 5.1 |
| train v2 prepare | ok | 5.9 |
| stress test report | ok | 0.1 |
...
```bash
conda run -n env_isaaclab python synthetic_smolvla/scripts/collect_oracle_demos.py --episodes 16 --output synthetic_smolvla/reports/oracle_acceptance_manifest.jsonl
```
...
```bash
conda run -n env_isaaclab python synthetic_smolvla/scripts/collect_oracle_demos.py --dataset-config synthetic_smolvla/configs/dataset_v1.yaml --episodes 16 --export-lerobot --overwrite
```
### train v1 prepare
...
```bash
conda run -n env_isaaclab python synthetic_smolvla/scripts/collect_oracle_demos.py --dataset-config synthetic_smolvla/configs/dataset_v2.yaml --episodes 32 --export-lerobot --overwrite
```
### train v2 prepare
...
```bash
/home/chyanin/miniconda3/envs/env_isaaclab/bin/python synthetic_smolvla/scripts/stress_test.py --episodes 64 --output synthetic_smolvla/reports/stress_test_v2.md
```
